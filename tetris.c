	...
	for(i=0;i<4;i++){
		for(j=0;j<4;j++){
			if (block[currentBlock][blockRotate][i][j]==1){
				if(i+blockY<0 || i+blockY>HEIGHT-1 || j+blockX<0 || j+blockX>WIDTH-1) return 0;
                else if(field[i+blockY][j+blockX]==1) return 0;
			}
		}
	}
    return 1;
...
void AddBlockToField(char field[HEIGHT][WIDTH],int currentBlock,int blockRotate, int blockY, int blockX){
	...
	for(i=0;i<4;i++){
		for(j=0;j<4;j++){
			if(block[currentBlock][blockRotate][i][j]==1){
                if(i+blockY<0 || i+blockY>HEIGHT-1) break;
                else if(j+blockX<0 || j+blockX>WIDTH-1) break;
                else field[blockY+i][blockX+j]=1;
            }
                //if(currentBlock[i][j]==1) field[blockY+i][blockX+j]=1;;
		}
	}
	// return ?
...
int DeleteLine(char field[HEIGHT][WIDTH]){
	// user code
	int i,j,check=0,line=0,value=0,k,l;
